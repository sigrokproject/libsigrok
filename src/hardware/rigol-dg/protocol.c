	        ...
	        const struct sr_channel_group *cg)
{
	...
	if (command && *command) {
		...
		if (!(s = params[0]))
			goto done;
		if (!g_ascii_strncasecmp(s, "SIN", 3))
			wf = WF_SINE;
		else if (!g_ascii_strncasecmp(s, "SQU", 3))
			wf = WF_SQUARE;
		else if (!g_ascii_strncasecmp(s, "RAMP", 4))
			wf = WF_RAMP;
		else if (!g_ascii_strncasecmp(s, "PULSE", 5))
			wf = WF_PULSE;
		else if (!g_ascii_strncasecmp(s, "NOISE", 5))
			wf = WF_NOISE;
		else if (!g_ascii_strncasecmp(s, "USER", 4))
			wf = WF_ARB;
		else if (!g_ascii_strncasecmp(s, "DC", 2))
			wf = WF_DC;
		else
			goto done;
		ch_status->wf = wf;
