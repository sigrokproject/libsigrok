...
static struct sr_dev_inst *probe_device(struct sr_scpi_dev_inst *scpi)
{
	...
	devc->config = g_malloc0(sizeof(struct per_channel_dev_context) * models[model_idx].channels);
	sdi->priv = devc;
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		case SR_CONF_CHANNEL_CONFIG:
			*data = g_variant_new_string((devc->tracking_enabled) ? "Track" : "Independent");
			break;
		...
		case SR_CONF_OVER_VOLTAGE_PROTECTION_THRESHOLD:
			*data = g_variant_new_double(devc->config[channel].over_voltage_protection_threshold);
			break;
		case SR_CONF_OVER_CURRENT_PROTECTION_THRESHOLD:
			*data = g_variant_new_double(devc->config[channel].over_current_protection_threshold);
			break;
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		case SR_CONF_ENABLED:
			...
			for (int chidx = 0 ; chidx < devc->model_config->channels ; ++chidx) {
				devc->config[chidx].output_enabled = bval;
				devc->config[chidx].mode_changed = TRUE;
			}
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_OUTPUT_ENABLE_DUAL, 0) < 0)
				return SR_ERR;
			break;
		case SR_CONF_CHANNEL_CONFIG:
			ch_config = g_variant_get_string(data, NULL);
			devc->tracking_enabled = (strcmp(ch_config, "Track") == 0);
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_TRACKING_ENABLE, 0) < 0)
				return SR_ERR;
			break;
		default:
			return SR_ERR_NA;
		...
		case SR_CONF_VOLTAGE_TARGET:
			...
			if (dval < devc->model_config->voltage[0] || dval > devc->model_config->voltage[1])
				return SR_ERR_ARG;
			if (devc->config[channel].voltage_target == dval)
				break;
			devc->config[channel].voltage_target = dval;
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_VOLTAGE_TARGET, channel) < 0)
				return SR_ERR;
			break;
		case SR_CONF_CURRENT_LIMIT:
			dval = g_variant_get_double(data);
			if (dval < devc->model_config->current[0] || dval > devc->model_config->current[1])
				return SR_ERR_ARG;
			if (devc->config[channel].current_limit == dval)
				break;
			devc->config[channel].current_limit = dval;
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_CURRENT_LIMIT, channel) < 0)
				return SR_ERR;
			break;
		case SR_CONF_ENABLED:
			bval = g_variant_get_boolean(data);
			...
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_OUTPUT_ENABLE, channel) < 0)
				return SR_ERR;
			break;
		case SR_CONF_OVER_VOLTAGE_PROTECTION_THRESHOLD:
			dval = g_variant_get_double(data);
			...
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_OVP_THRESHOLD, channel) < 0)
				return SR_ERR;
			break;
		case SR_CONF_OVER_CURRENT_PROTECTION_THRESHOLD:
			dval = g_variant_get_double(data);
			...
			if (aim_tti_dps_set_value(sdi->conn, devc, AIM_TTI_OCP_THRESHOLD, channel) < 0)
				return SR_ERR;
			break;
		default:
			return SR_ERR_NA;
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	if (!cg) {
		...
		case SR_CONF_SCAN_OPTIONS:
			return STD_CONFIG_LIST(key, data, sdi, cg, scanopts, drvopts, devopts_wotracking);
		case SR_CONF_DEVICE_OPTIONS:
			if (!devc || !devc->model_config)
				...
				return STD_CONFIG_LIST(key, data, sdi, cg, scanopts, drvopts, devopts_wtracking);
		case SR_CONF_CHANNEL_CONFIG:
			if (!devc || !devc->model_config)
				return SR_ERR_ARG;
