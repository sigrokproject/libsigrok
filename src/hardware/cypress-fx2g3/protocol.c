...
static int command_start_acquisition(const struct sr_dev_inst *sdi)
{
	...
	cmd.sampling_factor = (FX2G3_PIB_CLOCK)/(samplerate);
	cmd.channel_mask = devc->channel_mask;
	sr_err("cmd.sample rate = %d", samplerate);	
	sr_err("cmd.sampling_factor = %d",cmd.sampling_factor);
	sr_err("cmd.channel_mask = %d",cmd.channel_mask);
...
SR_PRIV int cypress_fx2g3_dev_open(struct sr_dev_inst *sdi, struct sr_dev_driver *di)
{
	...
	for (i = 0; i < device_count; i++) {
		 ...
		 */
		if (vi.major != FX2G3_REQUIRED_VERSION_MAJOR) {
			sr_err("Expected firmware version %d.x, "
			       "got %d.%d.", FX2G3_REQUIRED_VERSION_MAJOR,
			       vi.major, vi.minor);
			break;
		}
		...
		sr_info("Detected REVID, it's a Cypress FX2G3!\n");
...
SR_PRIV struct dev_context *cypress_fx2g3_dev_new(void)
{
	struct dev_context *devc;
...
SR_PRIV void cypress_fx2g3_abort_acquisition(struct dev_context *devc)
{
	int i;
...
static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
{
	...
	case LIBUSB_TRANSFER_NO_DEVICE:
		cypress_fx2g3_abort_acquisition(devc);
		free_transfer(transfer);
		return;
...
static int configure_channels(const struct sr_dev_inst *sdi)
{
	...
	sr_err(" configure_channels:num_analog = %d", num_analog);
	sr_err(" configure_channels:channel_mask = %d", channel_mask);
	/*
...
SR_PRIV int cypress_fx2g3_start_acquisition(const struct sr_dev_inst *sdi)
{
...
SR_PRIV int cypress_fx2g3_acquisition_stop(struct sr_dev_inst *sdi)
{
	command_stop_acquisition(sdi);
	cypress_fx2g3_abort_acquisition(sdi->priv);
