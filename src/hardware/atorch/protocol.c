...
static const uint8_t header_magic[] = { 0xff, 0x55 };

static const struct binary_analog_channel atorch_dc_power_meter_channels[] = {
	{ "V", { 4, BVT_BE_UINT24, 0.1, }, 1, SR_MQ_VOLTAGE, SR_UNIT_VOLT, SR_MQFLAG_DC },
	{ "I", { 7, BVT_BE_UINT24, 0.001, }, 3, SR_MQ_CURRENT, SR_UNIT_AMPERE, SR_MQFLAG_DC },
	{ "C", { 10, BVT_BE_UINT24, 0.01 }, 2, SR_MQ_ENERGY, SR_UNIT_AMPERE_HOUR, 0 },
	{ "E", { 13, BVT_BE_UINT32, 10.0, }, -1, SR_MQ_ENERGY, SR_UNIT_WATT_HOUR, 0 },
	{ "T", { 24, BVT_BE_UINT16, 1.0, },  0, SR_MQ_TEMPERATURE, SR_UNIT_CELSIUS, 0 },
	ALL_ZERO,
};

static const struct binary_analog_channel atorch_usb_power_meter_channels[] = {
	{ "V", { 4, BVT_BE_UINT24, 0.01, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT, SR_MQFLAG_DC },
	{ "I", { 7, BVT_BE_UINT24, 0.01, }, 2, SR_MQ_CURRENT, SR_UNIT_AMPERE, SR_MQFLAG_DC },
	{ "C", { 10, BVT_BE_UINT24, 0.001 }, 3, SR_MQ_ENERGY, SR_UNIT_AMPERE_HOUR, 0},
	{ "E", { 13, BVT_BE_UINT32, 0.01, }, 2, SR_MQ_ENERGY, SR_UNIT_WATT_HOUR, 0 },
	{ "D-", { 17, BVT_BE_UINT16, 0.01, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT, SR_MQFLAG_DC },
	{ "D+", { 19, BVT_BE_UINT16, 0.01, }, 2, SR_MQ_VOLTAGE, SR_UNIT_VOLT, SR_MQFLAG_DC },
	{ "T", { 21, BVT_BE_UINT16, 1.0, },  0, SR_MQ_TEMPERATURE, SR_UNIT_CELSIUS, 0 },
	ALL_ZERO,
};

static const struct atorch_device_profile atorch_profiles[] = {
	{ 0x02, "DC Meter", atorch_dc_power_meter_channels },
	{ 0x03, "USB Meter", atorch_usb_power_meter_channels },
};
...
static const uint8_t *receive_msg(struct sr_serial_dev_inst *serial, struct dev_context *devc)
{
	size_t len;
...
SR_PRIV int atorch_probe(struct sr_serial_dev_inst *serial, struct dev_context *devc)
{
	...
	start_time_us = g_get_monotonic_time();
	while (g_get_monotonic_time() - start_time_us < (ATORCH_PROBE_TIMEOUT_MS * 1000)) {
		msg_ptr = receive_msg(serial, devc);
		if (msg_ptr && msg_ptr[HEADER_MSGTYPE_IDX] == MSG_REPORT) {
			if ((p = find_profile_for_device_type(msg_ptr[PAYLOAD_START_IDX]))) {
				devc->profile = p;
				return SR_OK;
			}
			else {
				sr_err("Unrecognized device type(0x%.4" PRIx8 ")!", devc->buf[PAYLOAD_START_IDX]);
				return SR_ERR;
			}
		}
		g_usleep(100000);
