...
static int ols_convert_basic_trigger(const struct sr_dev_inst *sdi,
	...
	for (l = trigger->stages; l; l = l->next) {
		...
		for (m = stage->matches; m; m = m->next) {
			...
			case SR_TRIGGER_ONE:
				...
				break;
			default:
				sr_err("Unsupported trigger type: %d",
				       match->match);
				return SR_ERR;
			}
		}
...
SR_PRIV int ols_prepare_acquisition(const struct sr_dev_inst *sdi)
{
	...
	*/
	...
	if (!(devc->device_flags & DEVICE_FLAG_IS_DEMON_CORE)) {
		...
		if (ols_convert_basic_trigger(sdi, &basic_trigger_desc) !=
		    SR_OK) {
		...
		if (basic_trigger_desc.num_stages > 0) {
			delaycount =
				readcount * (1 - devc->capture_ratio / 100.0);
		...
		if (ols_convert_and_set_up_advanced_trigger(
			    sdi, &will_trigger) != SR_OK) {
 ...
 */
static int
ols_set_advanced_edge_trigger(struct sr_serial_dev_inst *serial,
			      ...
			      uint32_t neither_edge)
{
	...
	for (unsigned int i = 0; i < 16; i = i + 1) {
		...
		if (neither_edge & bitmask)
			...
			if (rising_edge & bitmask)
				lutbits |= OLS_ADV_TRIG_EDGERISE1;
			if (falling_edge & bitmask)
				lutbits |= OLS_ADV_TRIG_EDGEFALL1;
		}
		bitmask >>= 1;
		...
		if (neither_edge & bitmask)
			...
			if (rising_edge & bitmask)
				lutbits |= OLS_ADV_TRIG_EDGERISE0;
			if (falling_edge & bitmask)
				lutbits |= OLS_ADV_TRIG_EDGEFALL0;
		}
		bitmask >>= 1;
...
static int ols_set_advanced_trigger_timer(struct sr_serial_dev_inst *serial,
					  int timersel, /* 0 or 1 */
					  uint64_t count_10ns)
{
	int ret;
					...
					gboolean *will_trigger)
{
	...
	if (devc->capture_ratio > 0) {
		...
		uint64_t effective_divider =
			devc->capture_flags & CAPTURE_FLAG_DEMUX ?
				      (devc->cur_samplerate_divider + 1) / 2 :
				      (devc->cur_samplerate_divider + 1);
		uint64_t pretrigger_10ns_ticks =
			devc->limit_samples * effective_divider *
			devc->capture_ratio / 100 /* percent */;
	...
	if (ols_stage + num_req_trigger_stages > NUM_ADVANCED_TRIGGER_STAGES) {
		sr_err("Too many trigger stages: %d requested + %d internal > %d available",
		       num_req_trigger_stages, ols_stage,
		       NUM_ADVANCED_TRIGGER_STAGES);
		return SR_ERR;
	}

	sr_dbg("Setting OLS advanced trigger for %i stages",
	       num_req_trigger_stages);
	...
	for (const GSList *l = trigger->stages; l; l = l->next) {
		...
		if (level_mask) {
			if (num_stages_with_level_trigger >=
		...
		if (edge_rising | edge_falling) {
			...
			ols_set_advanced_edge_trigger(
				serial, num_stages_with_edge_trigger,
				edge_rising, edge_falling, 0U);
		...
		if (level_summing_input >= 0 &&
		    level_summing_input == edge_summing_input)
		...
		sr_spew(" Stage %d, lvl mask %.4x, edge %.4x, level term %d, "
			...
			ols_stage, level_mask, (edge_falling | edge_rising),
			current_level_term, current_edge_term, sum_inputs[0],
			sum_inputs[1], sum_inputs[2], sum_inputs[3],
