 ...
 *
 * Copyright (C) 2022-2024 Tomas Mudrunka <harviecz@gmail.com>
 *
...
int SDL_GetAudioDeviceSpec_open(int index, int iscapture, SDL_AudioSpec *spec)
{
	...
	if (!ret && spec->channels == 0) {
		...
		SDL_AudioDeviceID d;
		d = SDL_OpenAudioDevice(SDL_GetAudioDeviceName(index, iscapture),
			iscapture, spec, spec, SDL_AUDIO_ALLOW_ANY_CHANGE);
		if (d)
			SDL_CloseAudioDevice(d);
	}

	//This is hack to disable floating point samples before we figure out how to properly pass them to sigrok
	if(!ret && SDL_AUDIO_ISFLOAT(spec->format)) spec->format = AUDIO_S32SYS;
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	for (dev_i = 0; dev_i < dev_count; ++dev_i) {
		...
		sdi->model = g_strdup_printf("[#%d, %dch, %dHz] %s", dev_i,
			dev_spec.channels, dev_spec.freq, devc->sdl_device_name);
...
int sdl_data_callback(int fd, int revents, void *cb_data)
{
	...
	if (devc->limit_samples_remaining <=
		0 /* || devc->limit_samples_remaining > 65535 */) { //Already sent everything
	...
	if (SDL_AUDIO_ISFLOAT(sf))
		sr_err("Float samples are not really correctly implemented yet!");
	...
	if(packet_analog.num_samples) sr_session_send(sdi, &packet);
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	devc->sdl_device_spec.samples = SDL_BYTES_TO_SAMPLES(INPUT_BUFFER_SIZE,
		devc->sdl_device_spec);

	devc->sdl_device_handle = SDL_OpenAudioDevice(devc->sdl_device_name, 1,
		&devc->sdl_device_spec, NULL, 0);
	if (!devc->sdl_device_handle) {
		sr_err("Could not open device for capture!");
		return SR_ERR;
	}
	//SDL_ClearQueuedAudio(devc->sdl_device_handle);
