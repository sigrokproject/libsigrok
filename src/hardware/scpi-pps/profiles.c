		...
		unsigned int *num_channel_groups)
{
	...
	*/
	...
	if (channel_count > (int)ARRAY_SIZE(eez_psu_channel_names)) {
		/*
		 ...
		 */
		sr_warn(
			"Only first %d of %d channels of EEZ PSU are currently supported",
			channel_count, (int)ARRAY_SIZE(eez_psu_channel_names));
		channel_count = (int)ARRAY_SIZE(eez_psu_channel_names);
	}

	sr_spew("EEZ PSU (%s) has channel count %d", hw_info->model, channel_count);
