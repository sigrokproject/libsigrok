...
static void mastech_ms6514_data(struct sr_dev_inst *sdi, const uint8_t *buf)
{
	...
	if ((devc->data_source == DATA_SOURCE_MEMORY) && (mastech_ms6514_data_source(buf) == DATA_SOURCE_LIVE)) {
	...
	for (i = 0; i < MASTECH_MS6514_NUM_CHANNELS; i++) {
		...
		analog.meaning->channels = g_slist_append(NULL, g_slist_nth_data(sdi->channels, mastech_ms6514_channel_assignment(buf, i)));
