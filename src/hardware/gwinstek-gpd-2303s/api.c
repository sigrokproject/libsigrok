...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
  ...
  GMatchInfo *match_info;
	...
	if (gwinstek_gpd_2303s_receive_reply(serial, reply, sizeof(reply)) != SR_OK)
	{
  ...
  // returned identification string is for example "GW INSTEK,GPD-2303S,SN:ER915277,V2.10"
	...
	for (i = 0; i < ARRAY_SIZE(models); i++) {
		if (strcmp(g_match_info_fetch (match_info, 1), models[i].name) == 0) {
			model = &models[i];
			break;
		}
	}
	...
	gwinstek_gpd_2303s_receive_reply(serial, reply, sizeof(reply));
	{
	  ...
	  if (sscanf(reply, "%1u%1u%1u%1u%1u%1u%1u%1u", &cc_cv_ch1, &cc_cv_ch2, &track1, &track2, &beep, &devc->output_enabled, &baud1, &baud2) != 8)
		{
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	sr_info("config_get(%d, %s)", key, ki != NULL ? ki->name : NULL);
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	switch (key) {
		...
		case SR_CONF_VOLTAGE_TARGET:
			...
			if (dval < devc->model->channels[channel].voltage[0] || dval > devc->model->channels[channel].voltage[1])
				return SR_ERR_ARG;			
		...
		case SR_CONF_CURRENT_LIMIT:
			...
			dval = g_variant_get_double(data);
			if (dval < devc->model->channels[channel].current[0] || dval > devc->model->channels[channel].current[1])
				return SR_ERR_ARG;
...
SR_PRIV struct sr_dev_driver gwinstek_gpd_2303s_driver_info = {
	.name = "gwinstek-gpd-2303s",
	.longname = "GWInstek GPD-2303S",
	.api_version = 1,
	.init = std_init,
	.cleanup = std_cleanup,
	.scan = scan,
