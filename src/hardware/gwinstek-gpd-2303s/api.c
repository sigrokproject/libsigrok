...
static const char *const channel_modes[] = {
	"Independent",
};
...
static GSList *scan(struct sr_dev_driver *di, GSList *options)
{
	...
	sdi = g_malloc0(sizeof(const struct sr_dev_inst));
	...
	for (i = 0; i < model->num_channels; i++) {
		...
		cg = g_malloc(sizeof(const struct sr_channel_group));
	...
	devc = g_malloc0(sizeof(const struct dev_context));
	sr_sw_limits_init(&devc->limits);
	devc->model = model;
	devc->config = g_malloc0(sizeof(const struct per_channel_config)
				 * model->num_channels);
	sdi->priv = devc;
