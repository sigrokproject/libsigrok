...
static void rohde_schwarz_nrpxsn_send_packet(const struct sr_dev_inst *sdi, double value, int digits)
{
	...
	analog.encoding->digits = 3; /* unit is dBm so I can hardly believe more is accurate*/
...
//		else if(devc->sensitivity_changed) {
//			ret = rohde_schwarz_nrpxsn_update_sensitivity(scpi, devc);
//		}
		else {
...
SR_PRIV int rohde_schwarz_nrpxsn_init(struct sr_scpi_dev_inst *scpi, struct dev_context *devc)
{
	devc->measurement_state = IDLE;
...
SR_PRIV int rohde_schwarz_nrpxsn_update_trigger_source(struct sr_scpi_dev_inst *scpi, struct dev_context *devc)
{
	char *cmd;
	if (!scpi || !devc)
		return SR_ERR;

	cmd = (devc->trigger_source == 0)? /*INT*/ "TRIG:SOUR IMM" : "TRIG:SOUR EXT2";
...
SR_PRIV int rohde_schwarz_nrpxsn_update_curr_freq(struct sr_scpi_dev_inst *scpi, struct dev_context *devc)
{
	char cmd[32];
	if (!scpi || !devc)
		return SR_ERR;
