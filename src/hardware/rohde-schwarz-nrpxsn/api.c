...
/*
sigrok-cli --driver=rohde-schwarz-nrpxsn:conn=usbtmc/0aad.00e2 -l 4 --samples 4 --config center_frequency=2600000000
*/

static struct sr_dev_driver rohde_schwarz_nrpxsn_driver_info;
static const char *manufacturer = "ROHDE&SCHWARZ";

static const char *rohde_schwarz_nrpxsn_trigger_sources[] = {
	"INT", "EXT",
};

static const unsigned int rohde_schwarz_nrpxsn_num_trigger_sources = ARRAY_SIZE(rohde_schwarz_nrpxsn_trigger_sources);
...
static const uint32_t devopts[] = {
	...
	SR_CONF_CENTER_FREQUENCY | SR_CONF_GET | SR_CONF_SET, /* not downmixed but used for calibration-data-lookup */
};
...
static struct sr_dev_inst *probe_device(struct sr_scpi_dev_inst *scpi)
{
	...
	/*sr_scpi_send(scpi, "SYST:LOC"); // this device has no local interface*/
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_TRIGGER_SOURCE:
		if (!model)
			return SR_ERR_ARG;
		*data = g_variant_new_string(rohde_schwarz_nrpxsn_trigger_sources[devc->trigger_source]);
		break;
	default:
		return SR_ERR_NA;
	}
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_TRIGGER_SOURCE:
		if (!model || !scpi)
			return SR_ERR;
		if ((idx = std_str_idx(data, rohde_schwarz_nrpxsn_trigger_sources, rohde_schwarz_nrpxsn_num_trigger_sources)) < 0)
			return SR_ERR_ARG;
...
//		break;
	case SR_CONF_TRIGGER_SOURCE:
		if (!model)
			return SR_ERR_ARG;
		*data = g_variant_new_strv(rohde_schwarz_nrpxsn_trigger_sources, rohde_schwarz_nrpxsn_num_trigger_sources);
		break;
	default:
		return SR_ERR_NA;
	}
