...
static int ipdbg_awg_tcp_send(struct ipdbg_awg_tcp *tcp, const uint8_t *buf, size_t len)
{
	int out;
...
static int ipdbg_awg_send_escaping(struct ipdbg_awg_tcp *tcp, uint8_t *data_to_send,
	uint32_t length)
{
...
{
	uint8_t buf[8];
	uint8_t read_cmd = RETURN_SIZES_COMMAND;
...
static void ipdbg_awg_calculate_sine(struct dev_context *devc)
{
	...
	const double Om = 2.0*pi/(double)(devc->limit_samples);
	const double offset = devc->offset * full_scale;

	for (size_t i = 0 ; i < devc->limit_samples ; ++i)
		*out++ = (int64_t)round(A*sin(i*Om*devc->periods + devc->phase) + offset);
...
static void ipdbg_awg_calculate_triangle(struct dev_context *devc)
{
	...
	for (size_t i = 0 ; i < devc->limit_samples; ++i)
		*out++ = (int64_t)round(A*ipdbg_awg_triangle(i*N*devc->periods + phi, devc->dutycycle) + offset);
...
static void ipdbg_awg_calculate_rectangle(struct dev_context *devc)
{
	...
	for (size_t i = 0 ; i < devc->limit_samples; ++i)
		*out++ = (int64_t)round(A*ipdbg_awg_rectangle(i*N*devc->periods + phi, devc->dutycycle) + offset);
...
static void ipdbg_awg_update_limit_samples(struct dev_context *devc)
{
	if (devc->waveform == IPDBG_AWG_WAVEFORM_DC)
		...
		const size_t N = devc->limit_samples_max;
	...
	double a_value)
{
	...
	if (a_value > 2.0 || a_value < 0.0)
		return SR_ERR;

	devc->amplitude = a_value/2.0;
	...
	double p_value)
{
	...
	devc->phase = p_value/180.0*acos(-1.0);
...
{
	switch (waveform) {
		case IPDBG_AWG_WAVEFORM_DC:        return STR_WAVEFORM_DC;
		case IPDBG_AWG_WAVEFORM_SINE:      return STR_WAVEFORM_SINE;
		case IPDBG_AWG_WAVEFORM_RECTANGLE: return STR_WAVEFORM_RECTANGLE;
		case IPDBG_AWG_WAVEFORM_TRIANGLE:  return STR_WAVEFORM_TRIANGLE;
		//case IPDBG_AWG_WAVEFORM_NOISE:   return STR_WAVEFORM_NOISE;
		case IPDBG_AWG_WAVEFORM_ARB:       return STR_WAVEFORM_ARB;
	}
	return "Unknown";
}
