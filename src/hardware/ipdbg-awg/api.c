...
/** since the ipdbg-awg is used with different analog front-ends we use
  ...
  *
  * SR_CONF_AMPLITUDE: the value 2.0 (Vpp)  maps to the maximum
  ...
  * Triangle: the ratio of the rise time t_r to the half period t_h.
  *            0% -> falling ramp
...
/**
  * The ipdbg-awg is sometimes used to generate complex valued signals.
  * Either they are parallel at the output or time-multiplexed.
  * At the moment there is no SR_CONF_xyz parameter to select this...
  *
  * Are there any plans for an interface to feed the generators
  * in ARB mode with data? For example from one of the many source file
  * formats already supported by sigrok? Or a callback function(usable
  * from the bindings)?
  */
	...
	const struct sr_dev_inst *sdi, const struct sr_channel_group *cg)
{
	...
	case SR_CONF_PATTERN_MODE:
		*data = g_variant_new_string(ipdbg_awg_waveform_to_string(devc->waveform));
		break;
	default:
		return SR_ERR_NA;
	}
