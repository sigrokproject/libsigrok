...
static const uint32_t devopts[] = {
	SR_CONF_CONTINUOUS,
	SR_CONF_TIMEBASE | SR_CONF_SET |SR_CONF_GET| SR_CONF_LIST, // used to select gate time
	...
	case SR_CONF_TIMEBASE:
		{
			...
			if	    (p ==  10 && q == 1000) devc->gate_time = 0;
			else if (p == 100 && q == 1000) devc->gate_time = 1;
			else if (p ==   1 && q ==    1) devc->gate_time = 2;
			else if (p ==  10 && q ==    1) devc->gate_time = 3;
			else
				return SR_ERR_NA;
		}
		break;
