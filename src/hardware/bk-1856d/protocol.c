...
static void bk_1856d_select_input(const struct sr_dev_inst *sdi)
{
	...
	if (serial_write_blocking(serial, cmd, LENGHT_OF_CMD, serial_timeout(serial, LENGHT_OF_CMD)) < 1)
		sr_err("unable to send function %c command", devc->sel_input == InputA ? 'A' : 'C' );
...
static void bk_1856d_set_gate_time(const struct sr_dev_inst *sdi)
{
	struct dev_context *devc;
	struct sr_serial_dev_inst *serial;
	char *cmd;
	...
	switch(devc->gate_time) {
		default:
		case 0: cmd = GATE_TIME_0; sr_info("sending gate time 0 (10ms)");  microseconds = 40000;  break;
		case 1: cmd = GATE_TIME_1; sr_info("sending gate time 1 (100ms)"); microseconds = 80000;  break;
		case 2: cmd = GATE_TIME_2; sr_info("sending gate time 2 (1s)");    microseconds = 80000;  break;
		case 3: cmd = GATE_TIME_3; sr_info("sending gate time 3 (10s)");   microseconds = 800000; break;
	}
...
static void bk_1856d_request_data(const struct sr_dev_inst *sdi)
{
	...
	if (serial_write_blocking(serial, DATA_REQ, LENGHT_OF_CMD, serial_timeout(serial, LENGHT_OF_CMD)) < 1)
...
static void bk_1856d_send_packet(const struct sr_dev_inst *sdi, double freq_value, int digits)
{
...
SR_PRIV int bk_1856d_receive_data(int fd, int revents, void *cb_data)
{
	...
	{
		/* Timeout
		   in rare cases th bk1856 doesn't respond anymore (probably timing on rs232)
